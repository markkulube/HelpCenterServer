...
int add_student(Student **stu_list_ptr, char *student_name, char *course_code,
    ...
    new_student->name = malloc(sizeof(char)*strlen(student_name)+1);
...
void add_ta(Ta **ta_list_ptr, char *ta_name) {
    ...
    new_ta->name = malloc(sizeof(char)*strlen(ta_name)+1);
...
char *print_currently_serving(Ta *ta_list) {
    ...
    char *serve_buff = malloc(sizeof(char)*OUT_BUF_SIZE);
    ...
    if (ta_list == NULL) {
        ...
        char *msg1 = "No TAs are currently working.\n";
        strcat(serve_buff, msg1);
        
    } else {

        Ta *curr2 = ta_list;
        while (curr2 != NULL) {
            strncat(serve_buff, "TA: ", strlen("TA: "));

           if (curr2->current_student != NULL) {
                strncat(serve_buff, curr2->name, strlen(curr2->name) -1);
                strncat(serve_buff, " is serving ", strlen(" is serving ")-1);
                strcat(serve_buff, curr2->current_student->name);
                strcat(serve_buff, ".\n");

           } else {
                strncat(serve_buff, curr2->name, strlen(curr2->name)-1);
                strcat(serve_buff, " has no student.\n");
           }
           curr2 = curr2->next;
        }
    }
...
char* print_full_queue(Student *stu_list) {
    // printf("Full Queue\n");
    char *name_buff = malloc(sizeof(char)*OUT_BUF_SIZE);
    for (int i = 0; i < OUT_BUF_SIZE; i++)
    {
        name_buff[i]='\0';
    }

    char *msg1 = "Full Queue\n";
    Student *curr = stu_list;
    strcat(name_buff, msg1);
    

    while (curr != NULL) {
        strncat(name_buff, "Student ", strlen("Student "));
        strncat(name_buff, curr->name, strlen(curr->name)-1);
        strncat(name_buff, ": ", strlen(": ")-1);
        strncat(name_buff, curr->course->code, strlen(curr->course->code));
        strcat(name_buff, "\n");
